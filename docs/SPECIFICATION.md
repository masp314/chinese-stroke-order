...
> **更新注意（2026-07-15）:** 本書の基準日は2026-07-13、基準コミットは`5fd9240`であり、AI Extract、Cloudflare Worker/Gemini連携、CC-CEDICT辞書、My Vocabulary、AI利用回数・解除設定の追加前である。現在の製品仕様は、2026-07-14付の`docs/Hanzi_Steps_Product_Spec.docx`と現行ソースコードを優先する。本書はそれ以前の詳細技術仕様として参照すること。
...
## 5.11 PDF Worksheet生成

ステータス: **実装済み**

### 概要

現在の入力テキストから印刷用の中国語書き取り練習PDF（A4）を生成する。jsPDFによるクライアントサイド生成。CJK文字とピンイン（声調記号付き）はCanvas APIで描画し画像としてPDFに埋め込む。
...
1. **Cover page**: タイトル、レベル、対象語句一覧（中国語・ピンイン・英語意味）
2. **Practice pages**（各語句1ページ以上）:
   - Section 1: Trace each character — 大きなトレース枠（灰色ガイド文字）
   - Section 2: Trace, then write — 完全な語句単位での反復練習（最初の2回はトレース付き）
   - Section 3: Read aloud — チェックボックス
3. **Mini Test page**（オプション）: ピンインマッチング、語句コピー、自己チェック
...
最低回数は下限。ページに余白があればボックスが15mm以上を維持する範囲で追加する。

### 書き取り枠

- 1文字=1正方形、20mm（最小15mm）
- 田字格ガイド線（点線）
- トレース文字は薄い灰色
...
- ベクター描画、SVG、プリミティブ図形によるフェイクイラストは禁止
- 使用する場合はラスター画像（PNG/JPG/WebP）のみ
- MVP版ではイラストなし

### 既知の制約

- CJK文字はcanvas描画→画像埋め込みのためベクターではない
- PDFの文字検索でCJK文字はヒットしない
- 大量の語句でPDFサイズが大きくなる（1語あたり約350KB）
- jsPDFの標準フォントでは拡張ラテン文字（ǎ, ǐ等）が正しく表示されないためピンインもcanvas画像
- フォントの見た目はブラウザ/OSのシステムフォントに依存

### コンポーネント

| ファイル | 責務 |
|---|---|
| `WorksheetPanel.tsx` | 語句編集UI、オプション選択、PDF生成トリガー |
| `worksheetItems.ts` | 入力テキストからWorksheetItem[]生成、CC-CEDICT辞書連携 |
| `worksheetPdf.ts` | jsPDFによるPDF生成（Cover / Practice / Mini Test） |
| `types/worksheet.ts` | WorksheetItem, WorksheetOptions型定義 |
...
| PDF Worksheet生成 | 実装済み | jsPDF、CJK文字はcanvas描画、英語意味はCC-CEDICT |
| 画像OCR | 一部実装 | 精度保証なし |
